...
- `engineVersion` is stamped on every `EngineDocument` (`ENGINE_VERSION`, currently `0.2.1.0`). Format is `major.minor.patch.build`. **Compatibility epoch is `major.minor`**. `engineDocumentFromJSON` rejects missing or foreign epochs. Use `migrateEngineStateToDocument` for 0.1 bare-state saves.
...
- Derived values (stats / pool maxima / pool reserved from tags) live in engine selectors; UniversalTags merge into active-game evaluation. Stored pool values are **Available** only.
...
- Tag passives: `stat`, `pool-max`, `generate-pool`, `reserve-pool`, continuous-* 
