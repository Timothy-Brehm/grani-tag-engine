...
describe('entity metrics', () => {
  ...
  it('counts manual and automatic action executions on the actor', () => {
    const action: ActionDefinition = {
      ...
      requiredFinishedEffects: [{ type: 'grant-tag', name: 'crafted', strength: 1 }],
    ...
    state = reduceEngineState(
      state,
      {
        ...
        action: {
          ...action,
          requiredFinishedEffects: [{ type: 'grant-tag', name: 'crafted-2', strength: 1 }],
        },
        actorEntityId: 'player',
        execution: 'automatic',
      },
  ...
  it('stamps tick times for actions, tags, and pool spends', () => {
    const action: ActionDefinition = {
      ...
      requirements: [{ type: 'free' }],
      requiredImmediateEffects: [],
      requiredFinishedEffects: [{ type: 'grant-tag', name: 'scouted', strength: 1 }],
      optionalFinishedEffects: [],
    };
    let state = createPrimaryEngineState(
      createEntityInstance({
